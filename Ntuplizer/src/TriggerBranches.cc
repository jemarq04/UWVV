...
TriggerBranch::TriggerBranch(const std::string& name,
                             ...
                             bool checkPrescale,
                             bool ignoreMissing)
    : name(name), checkPrescale(checkPrescale), ignoreMissing(ignoreMissing) {
  if (!pathExps.size())
    throw cms::Exception("BadTriggerPath") << "Trigger branch must contain at least one path." << std::endl;
      ...
      if (path.prescale(*prescales) != prescale) {
        if (checkPrescale)
          throw cms::Exception("InvalidPrescale")
              ...
              << "(" << paths.at(0).name() << " and " << path.name() << ")" << std::endl;
      }
...
TriggerBranches::TriggerBranches(edm::ConsumesCollector cc, const edm::ParameterSet& config, TTree* const tree)
    : resultsToken(cc.consumes<edm::TriggerResults>(config.getParameter<edm::InputTag>("trigResultsSrc"))),
      prescalesToken(cc.consumes<pat::PackedTriggerPrescales>(
          config.exists("trigPrescaleSrc") ? config.getParameter<edm::InputTag>("trigPrescaleSrc")
                                           : edm::InputTag("patTrigger"))),
      isValid(false),
      checkPrescale(config.exists("checkPrescale") ? config.getParameter<bool>("checkPrescale") : true) {
  std::vector<std::string> names = config.getParameter<std::vector<std::string> >("trigNames");
  bool ignoreMissing(config.getUntrackedParameter<bool>("ignoreMissing", false));

  for (auto& name : names) {
    std::vector<std::string> paths = config.getParameter<std::vector<std::string> >(name + "Paths");
...
void TriggerBranches::fill() {
  if (!isValid)
    throw cms::Exception("UninitializedTriggerBranches")
        << "ERROR: attempt to use uninitialized TriggerBranches object." << std::endl;
