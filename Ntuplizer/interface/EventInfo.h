...
namespace uwvv {
  ...
  class EventDatum {
  public:
    EventDatum(edm::ConsumesCollector& cc, const edm::InputTag& tag) : token_(cc.consumes<T>(tag)), isValid_(false) {
      ;
    }
    ~EventDatum() { ; }
  ...
  template <class T>
  using DatumPtr = std::unique_ptr<EventDatum<T> >;

  template <class T>
  class EventInfoHolder {
  public:
    EventInfoHolder(edm::ConsumesCollector& cc, const edm::InputTag& primaryTag, const edm::ParameterSet& moreTags);
    ~EventInfoHolder() { ; }
  ...
  public:
    ...
    const edm::Ptr<reco::Vertex> pv() { return (nVertices() ? vertices()->ptrAt(0) : edm::Ptr<reco::Vertex>(NULL, 0)); }
    ...
    const edm::Handle<edm::View<reco::Vertex> >& vertices(const std::string& collection) {
      return vertices_.get(collection);
    }
    const edm::Handle<edm::View<pat::Electron> >& electrons() { return electrons_.get(); }
    const edm::Handle<edm::View<pat::Electron> >& electrons(const std::string& collection) {
      return electrons_.get(collection);
    }
    ...
    const edm::Handle<edm::View<pat::Photon> >& photons(const std::string& collection) {
      return photons_.get(collection);
    }
    ...
    const edm::Handle<edm::View<pat::PackedCandidate> >& pfCands(const std::string& collection) {
      return pfCands_.get(collection);
    }
    ...
    const edm::Handle<std::vector<PileupSummaryInfo> >& puInfo(const std::string& collection) {
      return puInfo_.get(collection);
    }
    const edm::Handle<GenEventInfoProduct>& genEventInfo() { return genEventInfo_.get(); }
    const edm::Handle<GenEventInfoProduct>& genEventInfo(const std::string& collection) {
      return genEventInfo_.get(collection);
    }
    const edm::Handle<LHEEventProduct>& lheEventInfo() { return lheEventInfo_.get(); }
    const edm::Handle<LHEEventProduct>& lheEventInfo(const std::string& collection) {
      return lheEventInfo_.get(collection);
    }
    ...
    const edm::Handle<edm::View<reco::GenJet> >& genJets(const std::string& collection) {
      return genJets_.get(collection);
    }
    const edm::Handle<edm::View<reco::GenParticle> >& genParticles() { return genParticles_.get(); }
    const edm::Handle<edm::View<reco::GenParticle> >& genParticles(const std::string& collection) {
      return genParticles_.get(collection);
    }
    const edm::Handle<edm::View<pat::CompositeCandidate> >& initialStates() { return initialStates_.get(); }
    const edm::Handle<edm::View<pat::CompositeCandidate> >& initialStates(const std::string& collection) {
      return initialStates_.get(collection);
    }
    const edm::Handle<edm::View<pat::CompositeCandidate> >& genInitialStates() { return genInitialStates_.get(); }
    const edm::Handle<edm::View<pat::CompositeCandidate> >& genInitialStates(const std::string& collection) {
      return genInitialStates_.get(collection);
    }
