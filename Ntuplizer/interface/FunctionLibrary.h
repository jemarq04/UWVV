...
namespace {
  ...
  struct GeneralFunctionList {
    ...
    static void addFunctions(
        std::unordered_map<std::string, std::function<B(const edm::Ptr<T>&, uwvv::EventInfo&, const std::string&)> >&
            addTo) {
      ;
    }
  };
  ...
  struct GeneralFunctionList<std::vector<float> > {
    ...
    static void addFunctions(
        std::unordered_map<std::string,
                           std::function<std::vector<float>(const edm::Ptr<T>&, uwvv::EventInfo&, const std::string&)> >&
            addTo) {
      ...
      addTo["lheWeights"] =
          std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
            ...
            if (!evt.lheEventInfo().isValid())
              throw cms::Exception("ProductNotFound") << "Unable to open LHE event information";
            ...
            if (option != "") {
              ...
              if (pos == std::string::npos)
                try {
                  ...
                  std::string message = "Unable to parse option " + option + " for LHE weights. Error from ";
  ...
  struct GeneralFunctionList<float> {
    ...
    static void addFunctions(
        std::unordered_map<std::string,
                           std::function<float(const edm::Ptr<T>&, uwvv::EventInfo&, const std::string&)> >& addTo) {
      typedef float(FType)(const edm::Ptr<T>&, uwvv::EventInfo&, const std::string&);

      addTo["pvZ"] = std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
      ...
      addTo["nTruePU"] =
          std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
            return (evt.puInfo().isValid() && evt.puInfo()->size() > 0 ? evt.puInfo()->at(1).getTrueNumInteractions()
                                                                       : -1.);
          });
      ...
      addTo["L1prefiringWeightUp"] =
          std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
            ...
            if (*(evt.prefweightup()) >= 0.) {
              L1PrefWeightUp = *(evt.prefweightup());
            }
      ...
      addTo["L1prefiringWeightDn"] =
          std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
            ...
            if (*(evt.prefweightdown()) >= 0.) {
              L1PrefWeightDn = *(evt.prefweightdown());
            }
            return (L1PrefWeightDn);
          });
      addTo["type1_pfMETEt"] = std::function<FType>(
          [](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) { return evt.met().pt(); });

      addTo["type1_pfMETPhi"] = std::function<FType>(
          [](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) { return evt.met().phi(); });
      ...
      addTo["zeppenfeldGen"] =
          std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
            ...
            for (size_t i = 0; i < evt.genJets(option)->size(); ++i) {
              ...
              if (!uwvv::helpers::overlapWithAnyDaughter(j, *obj, 0.4)) {
                if (j1)
                  return std::abs(obj->rapidity() - (j1->rapidity() + j.rapidity()) / 2.);
                else
                  j1 = &j;
              }
            }
      ...
      addTo["zeppenfeldj3Gen"] =
          std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
            ...
            for (size_t i = 0; i < evt.genJets(option)->size(); ++i) {
              ...
              if (!uwvv::helpers::overlapWithAnyDaughter(j, *obj, 0.4)) {
                if (j2)
                  return std::abs(j.rapidity() - (j1->rapidity() + j2->rapidity()) / 2.);
                else if (j1)
                  j2 = &j;
                else
                  j1 = &j;
  ...
  struct GeneralFunctionList<bool> {
    template <class T>
    static void addFunctions(
        std::unordered_map<std::string, std::function<bool(const edm::Ptr<T>&, uwvv::EventInfo&, const std::string&)> >&
            addTo) {
  ...
  struct GeneralFunctionList<int> {
    template <class T>
    static void addFunctions(
        std::unordered_map<std::string, std::function<int(const edm::Ptr<T>&, uwvv::EventInfo&, const std::string&)> >&
            addTo) {
      typedef int(FType)(const edm::Ptr<T>&, uwvv::EventInfo&, const std::string&);

      addTo["Charge"] = std::function<FType>(
          [](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) { return obj->charge(); });

      addTo["PdgId"] = std::function<FType>(
          [](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) { return obj->pdgId(); });
    }
  };
  ...
  struct GeneralFunctionList<unsigned> {
    ...
    static void addFunctions(
        std::unordered_map<std::string,
                           std::function<unsigned(const edm::Ptr<T>&, uwvv::EventInfo&, const std::string&)> >& addTo) {
      typedef unsigned(FType)(const edm::Ptr<T>&, uwvv::EventInfo&, const std::string&);

      addTo["lumi"] = std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
        return evt.id().luminosityBlock();
      });

      addTo["run"] = std::function<FType>(
          [](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) { return evt.id().run(); });

      addTo["nvtx"] = std::function<FType>(
          [](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) { return evt.nVertices(); });
  ...
  struct GeneralFunctionList<unsigned long long> {
    ...
    static void addFunctions(
        std::unordered_map<std::string,
                           std::function<unsigned long long(const edm::Ptr<T>&, uwvv::EventInfo&, const std::string&)> >&
            addTo) {
      typedef unsigned long long(FType)(const edm::Ptr<T>&, uwvv::EventInfo&, const std::string&);

      addTo["evt"] = std::function<FType>(
          [](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) { return evt.id().event(); });
    }
  };
  ...
  struct ObjectFunctionList<float, pat::Electron> {
    ...
    static void addFunctions(std::unordered_map<std::string, std::function<FType> >& addTo) {
      ...
      addTo["IP3D"] = std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
        return fabs(obj->dB(T::PV3D));
      });

      addTo["IP3DUncertainty"] = std::function<FType>(
          [](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) { return obj->edB(T::PV3D); });
      ...
      addTo["IP2D"] = std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
        return fabs(obj->dB(T::PV2D));
      });

      addTo["IP2DUncertainty"] = std::function<FType>(
          [](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) { return obj->edB(T::PV2D); });

      addTo["PVDZ"] = std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
  ...
  struct ObjectFunctionList<unsigned, pat::Muon> {
    ...
    static void addFunctions(std::unordered_map<std::string, std::function<FType> >& addTo) {
      addTo["BestTrackType"] =
          std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
            return obj->muonBestTrackType();
          });

      addTo["MatchedStations"] =
          std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
            return obj->numberOfMatchedStations();
          });
  ...
  struct ObjectFunctionList<float, pat::CompositeCandidate> {
    ...
    static void addFunctions(std::unordered_map<std::string, std::function<FType> >& addTo) {
      addTo["mjj"] = std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
      ...
      addTo["jetPUSFmulfac"] =
          std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
            if (!obj->hasUserFloat("jetPUSFmulfac")) {
              throw cms::Exception("ProductNotFound") << "jet PUSF mulfac not found"
                                                      << "for variation " << option;
            }

            return (obj->userFloat("jetPUSFmulfac"));
          });
      addTo["ptjj"] = std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
        const edm::PtrVector<pat::Jet>* cleanedJets = uwvv::helpers::getCleanedJetCollection(*obj, option);
        if (cleanedJets->size() < 2)
          return -999.;
      ...
      addTo["zeppenfeld"] =
          std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
            ...
            return std::abs(obj->rapidity() - ((*cleanedJets)[0]->rapidity() + (*cleanedJets)[1]->rapidity()) / 2.);
          });
      ...
      addTo["zeppenfeldj3"] =
          std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
            ...
            return std::abs((*cleanedJets)[2]->rapidity() -
                            ((*cleanedJets)[0]->rapidity() + (*cleanedJets)[1]->rapidity()) / 2.);
          });
      ...
      addTo["DR"] = std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
        return reco::deltaR(obj->daughter(0)->p4(), obj->daughter(1)->p4());
      });
    ...
    static void addFunctions(std::unordered_map<std::string, std::function<FType> >& addTo) {
      ...
      addTo["jetDeepCSV"] =
          std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
            ...
            for (auto& jet : *uwvv::helpers::getCleanedJetCollection(*obj, option)) {
              out.push_back(jet->bDiscriminator("pfDeepCSVJetTags:probb") +
  ...
  struct ObjectFunctionList<bool, pat::CompositeCandidate> {
    ...
    static void addFunctions(std::unordered_map<std::string, std::function<FType> >& addTo) {
      addTo["SS"] = std::function<FType>([](const edm::Ptr<T>& obj, uwvv::EventInfo& evt, const std::string& option) {
...
namespace uwvv {
  ...
  public:
    ...
    std::function<FSig> getFunction(const std::string& f) const {
      ...
      return std::bind(functions.at(fname), std::placeholders::_1, std::placeholders::_2, option);
    }
  ...
  protected:
    std::unordered_map<std::string, std::function<FType> > functions;
  };
  ...
  public:
    ...
    std::function<FSig> getFunction(const std::vector<std::string>& fs) const {
      if (fs.size() == 1) {
      ...
      auto out = std::function<FSig>([needed](const edm::Ptr<T>& obj, uwvv::EventInfo& evt) {
        std::vector<B> out;
