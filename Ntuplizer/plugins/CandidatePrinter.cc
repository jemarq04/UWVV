...
private:
  virtual void analyze(edm::Event const& evt, edm::EventSetup const& setup);

  void printCandInfo(const reco::Candidate* cand, size_t level) const;
  void printDaughterInfo(const CCand* cand, size_t level) const;
...
void CandidatePrinter::analyze(edm::Event const& event, edm::EventSetup const& setup) {
  ...
  if (candsIn->size())
    std::cout << event.id().run() << ":" << event.id().luminosityBlock() << ":" << event.id().event() << ":"
...
void CandidatePrinter::printCandInfo(const reco::Candidate* cand, size_t level) const {
  if (!cand) {
  ...
  std::cout << "id: " << cand->pdgId() << "m: " << cand->mass() << " pt: " << cand->pt() << " eta: " << cand->eta()
            << " phi: " << cand->phi() << " ((" << cand->numberOfDaughters() << "))" << std::endl;

  if (cand->numberOfDaughters()) {
    const CCand* castCand = (cand->hasMasterClone() ? dynamic_cast<const CCand*>(cand->masterClone().get())
...
void CandidatePrinter::printDaughterInfo(const CCand* cand, size_t level) const {
