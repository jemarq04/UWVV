...
def writeFarmoutCommand(
    ...
    **args,
    ...
    for _ in range(5):
        ...
        raise RuntimeError("Failed to get file list from DAS. Check connection to client.")
    ...
    if args.get("applyLumiMask", False):
        ...
        lumiMask = os.path.join(defaultJSONPath, lumiMask)
...
def buildScript(
    ...
    **args,
    ...
    for _ in range(5):
        ...
        raise RuntimeError("Failed to get dataset list from DAS. Check connection to client.")
    ...
    for s in samples:
        ...
        for match in matches:
            ...
            lines.append(writeFarmoutCommand(cfg, jobid, name, match, outdir, **args))
...
if __name__ == "__main__":
    ...
    parser.add_argument("jobid", type=str, help="Unique identifier for set of jobs.")
    parser.add_argument("cfg", type=str, help="CMS config file to run.")
    parser.add_argument("--applyLumiMask", action="store_true", help="Pass the appropriate lumi-mask JSON (data only).")
    parser.add_argument(
        "--lumiMaskJSON",
        type=str,
        ...
        help=("Lumi mask JSON. Assumed to be in the standard certification area unless a full path is given."),
    )
    parser.add_argument("--campaign", type=str, default="", help="MC generation campaign for samples (MC only).")
    parser.add_argument("--dataEra", type=str, default="", help="Run over data.")
    parser.add_argument("--samples", nargs="+", type=str, required=False, help="Samples to run. May contain wildcards.")
    parser.add_argument(
        "--outdir",
        type=str,
    ...
    parser.add_argument("--filesPerJob", type=int, default=1, help="Number of input files to pass to each job.")
    parser.add_argument(
        "--extraUsercodeFiles",
        nargs="*",
        type=str,
    ...
    parser.add_argument("cmsRunArgs", nargs=argparse.REMAINDER, help="Arguments to cmsRun.")
    ...
    buildScript(
        ...
        args.outdir,
        *args.samples,
        applyLumiMask=args.applyLumiMask,
        lumiMaskJSON=args.lumiMaskJSON,
