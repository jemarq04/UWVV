...
def writeFarmoutCommand(
    ...
    **args,
    ...
    if os.path.exists(submitDir):
        ...
        return "# Submit directory for sample {} already exists, skipping".format(
            dataset
        )
    ...
    for _ in range(5):
        ...
        raise RuntimeError(
            "Failed to get file list from DAS. Check connection to client."
        )
    ...
    cmds = [
        ...
        '"--output-dir={}"'.format(
            outdir.format(user=user, jobid=jobid, dataset=dataset)
        ),
    ...
    if args.get("extraUsercodeFiles", []):
        cmds.append(
            '--extra-usercode-files="{}"'.format(
                " ".join(args.get("extraUsercodeFiles"))
            )
        )
    ...
    if args.get("applyLumiMask", False):
        ...
        defaultJSONPath = (
            "/afs/cern.ch/cms/CAF/CMSCOMM/COMM_DQM/certification/Collisions17/13TeV"
        )
...
def buildScript(
    ...
    **args,
    ...
    for _ in range(5):
        ...
        raise RuntimeError(
            "Failed to get dataset list from DAS.Check connection to client."
        )
    ...
    if not lines:
        log.error(
            "No datasets found matching {}, script will be empty!".format(samples)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            "Lumi mask JSON. Assumed to be in the standard "
    ...
    parser.add_argument(
        "cmsRunArgs", nargs=argparse.REMAINDER, help="Arguments to cmsRun."
    )
