...
def resubmit(sample, dryrun=False, quiet=False):
    ...
    try:
        ...
        inProgress = results["Pre"] + results["Post"] + results["Queued"] + results["Ready"]
    ...
    if failed or not quiet:
        _log.info("    " + sample)
        _log.info(
            "        Total: {0} Done: {1} Queued: {2} Failed: {3}".format(total - ignore, succeeded, inProgress, failed)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dry-run", dest="dryrun", action="store_true", help="Show samples to submit without submitting them"
    )
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="Only print information about samples with failed jobs"
    )
