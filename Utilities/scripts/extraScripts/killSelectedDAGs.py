...
logging.basicConfig(level=logging.INFO, stream=stderr, format="%(message)s")
...
def killSelectedDAGs(*paths, **kwargs):
    ...
    for path in paths:
        ...
        for p in glob(fullPath):
            ...
            if dryRun:
                log.info(
                    "Would kill {} with cluster ID {} if this weren't a dry run".format(
                        "/".join(p.split("/")[-2:]), cluster
                    )
                )
                continue
