...
parser.add_argument(
    "--report", action="store_true", help="if provided, also run 'crab report'"
)
parser.add_argument(
    "--noprocessing", action="store_true", help="if provided, skip processing"
)
parser.add_argument(
    "--nowriting",
    action="store_true",
    help="if provided, skip writing output info file",
)
parser.add_argument(
    "--noresubmit",
    action="store_true",
    help="if provided, skip creating resubmit script",
)
parser.add_argument(
    "-d", "--outdir", default="output_crab_status_data", help="output status folder"
)
parser.add_argument(
    "-o", "--outname", default="status_info.txt", help="output info txt file name"
)
parser.add_argument(
    "dir", default="./", nargs="?", help="input directory to search for crab folders"
)
...
print(
    "Running scripts. Don't forget to initialize proxy first.\nSee the latest folder/files with largest index."
)
...
if not args.noprocessing:
    ...
    for folder in crablist:
        command = "crab status -d %s > %s.txt 2>&1" % (
            folder,
            os.path.join(args.outdir, folder),
        )
        code = subprocess.call([command], shell=True)
        if args.report:
            command2 = "crab report -d %s > %s_Report.log 2>&1" % (
                folder,
                os.path.join(args.outdir, folder),
            )
