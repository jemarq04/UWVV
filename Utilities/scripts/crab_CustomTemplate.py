...
if isMC:
    if any(
        generator in primaryDS.lower() for generator in ["mcfm", "phantom", "sherpa"]
    ):
        lheWeight = 0
    else:
...
if isMC:
    config.General.requestName = "_".join(
        [campaign_name, localSettings.get("local", "requestName")]
    )
...
config.Data.outLFNDirBase = (
    localSettings.get("local", "outLFNDirBase")
