...
for arg in sys.argv:
    ...
    if "dataset" in localSettings:
        ...
        raise Exception(
            "Must pass dataset argument as Data.inputDataset=... or include dataset argument in config file"
        )
...
if customMC:
    ...
    if any(name not in localSettings for name in ["requestName", "datalist"]):
        raise Exception(
            "Custom jobs require three extra options: dataset, requestName, and datalist."
        )
...
if year == "2022":
    if "postEE" in localSettings:
        ...
        postEE = (
            1
            if any("Run2022%s" % subera in conditions for subera in ["E", "F", "G"])
            else 0
        )
    else:
...
if not customMC:
    ...
    with open(localSettings["datalist"], "r") as infile:
        config.Data.userInputFiles = [
            line for line in infile.readlines() if line and line[0] != "#"
        ]
...
if isMC:
    config.General.requestName = "_".join(
        [campaign_name, primaryDS if not customMC else localSettings["requestName"]]
    )
    ...
    if year == "2022":
        ...
        exit()
    config.Data.lumiMask = "%s/src/UWVV/Utilities/scripts/JSON/%s" % (
...
config.JobType.psetName = (
    "%s/src/UWVV/Ntuplizer/test/ntuplize_cfg.py" % os.environ["CMSSW_BASE"]
)
