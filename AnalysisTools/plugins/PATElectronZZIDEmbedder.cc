...
PATElectronZZIDEmbedder::PATElectronZZIDEmbedder(const edm::ParameterSet& iConfig)
    : srcToken_(consumes<ElectronView>(iConfig.exists("src") ? iConfig.getParameter<edm::InputTag>("src")
      ...
      idLabel_(iConfig.exists("idLabel") ? iConfig.getParameter<std::string>("idLabel") : std::string("HZZ4lIDPass")),
      isoLabel_(iConfig.exists("isoLabel") ? iConfig.getParameter<std::string>("isoLabel")
      ...
      idCutLowPtLowEta(iConfig.exists("idCutLowPtLowEta") ? iConfig.getParameter<double>("idCutLowPtLowEta")
                                                          : 0.9044286167),  //EB1_5
      idCutLowPtMedEta(iConfig.exists("idCutLowPtMedEta") ? iConfig.getParameter<double>("idCutLowPtMedEta")
                                                          : 0.9094166886),  //EB2_5
      idCutLowPtHighEta(iConfig.exists("idCutLowPtHighEta") ? iConfig.getParameter<double>("idCutLowPtHighEta")
                                                            : 0.9443653660),  //EE_5
      idCutHighPtLowEta(iConfig.exists("idCutHighPtLowEta") ? iConfig.getParameter<double>("idCutHighPtLowEta")
                                                            : 0.1968600840),  //EB1_10
      idCutHighPtMedEta(iConfig.exists("idCutHighPtMedEta") ? iConfig.getParameter<double>("idCutHighPtMedEta")
                                                            : 0.0759172100),  //EB2_10
      idCutHighPtHighEta(iConfig.exists("idCutHighPtHighEta") ? iConfig.getParameter<double>("idCutHighPtHighEta")
                                                              : -0.5169136775),  //EE_10
      bdtLabel(iConfig.exists("bdtLabel") ? iConfig.getParameter<std::string>("bdtLabel")
      ...
      cutBasedLabel(iConfig.exists("cutBasedLabel") ? iConfig.getParameter<std::string>("cutBasedLabel")
                                                    : "cutBasedElectronID-RunIIIWinter22-V1"),
...
bool PATElectronZZIDEmbedder::passVertex(const Electron& ele) const {
  return (fabs(ele.dB(Electron::PV3D) / ele.edB(Electron::PV3D)) < sipCut && fabs(ele.dB(Electron::PV2D)) < pvDXYCut &&
