...
typedef pat::CompositeCandidateCollection CCandCollection;
...
private:
  ...
  const std::vector<std::string> makePairNames(const std::vector<std::string>& names) const;
  ...
  // user cands
  template <class T>
  const edm::Ptr<reco::Candidate> getFSRCand(const T& obj) const;
...
AlternateDaughterInfoEmbedder::AlternateDaughterInfoEmbedder(const edm::ParameterSet& iConfig)
    : srcToken(consumes<CCandView>(iConfig.getParameter<edm::InputTag>("src"))),
      names(iConfig.getParameter<std::vector<std::string> >("names")),
      pairNames(makePairNames(names)),
      nDaughters(names.size()),
      fsrLabel(iConfig.exists("fsrLabel") ? iConfig.getParameter<std::string>("fsrLabel") : "") {
  if (nDaughters < 3)
    throw cms::Exception("InvalidChannel") << "AlternateDaughterEmbedder "
                                           << "can't do anything with fewer than 3 particles in the final state!";

  for (auto& name : names) {
    if (fsrLabel.empty())
      fsrGetters.push_back(
          std::function<FType>([](const reco::Candidate* const obj) { return edm::Ptr<reco::Candidate>(NULL, 0); }));
      ...
      if (name.find('m') == std::string::npos)
        throw cms::Exception("BadInput") << "Alternate DaughterInfoEmbedder currently only supports e and mu"
                                         << std::endl;
  ...
  produces<CCandCollection>();
}

void AlternateDaughterInfoEmbedder::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  edm::Handle<CCandView> in;
  iEvent.getByToken(srcToken, in);

  std::unique_ptr<CCandCollection> out(new CCandCollection);
...
const std::vector<std::string> AlternateDaughterInfoEmbedder::makePairNames(
    const std::vector<std::string>& daughterNames) const {
...
template <class T>
const edm::Ptr<reco::Candidate> AlternateDaughterInfoEmbedder::getFSRCand(const T& obj) const {
