...
template<typename T>
class GenObjectCounter : public edm::stream::EDProducer<>
{
  public:
    explicit GenObjectCounter(const edm::ParameterSet& iConfig);
    virtual ~GenObjectCounter() {};
...
template<typename T>
GenObjectCounter<T>::GenObjectCounter(const edm::ParameterSet& iConfig) :
...
template<typename T>
void GenObjectCounter<T>::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (size_t i=0; i<in->size(); i++){
    //std::cout << in->at(i).mother(0)->pdgId() << std::endl;
  }
}

typedef GenObjectCounter<reco::GenParticle> GenParticleCounter;
typedef GenObjectCounter<reco::GenJet> GenJetCounter;
typedef GenObjectCounter<pat::PackedGenParticle> PackedGenParticleCounter;

#include "FWCore/Framework/interface/MakerMacros.h"
DEFINE_FWK_MODULE(GenParticleCounter);
DEFINE_FWK_MODULE(GenJetCounter);
DEFINE_FWK_MODULE(PackedGenParticleCounter);
