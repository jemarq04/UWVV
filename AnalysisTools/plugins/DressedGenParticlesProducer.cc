...
DressedGenParticlesProducer::DressedGenParticlesProducer(const edm::ParameterSet& iConfig)
    : baseCollectionToken_(
          consumes<reco::GenParticleCollection>(iConfig.getParameter<edm::InputTag>("baseCollection"))),
      associatesToken_(consumes<reco::GenParticleCollection>(iConfig.getParameter<edm::InputTag>("associates"))),
...
bool DressedGenParticlesProducer::allUniqueAssociates(reco::CandidateCollection& dressedParticles) {
  ...
  for (const auto& particle : dressedParticles) {
    const DressedGenParticle& dressed_part = dynamic_cast<const DressedGenParticle&>(particle);
    reco::GenParticleCollection associated = dressed_part.getAssociated();
    allAssociated.insert(allAssociated.end(), associated.begin(), associated.end());
  }
  std::set<reco::GenParticle, GreaterByPt<reco::Candidate>> uniqueAssociated(allAssociated.begin(),
                                                                             allAssociated.end());
  if (allAssociated.size() != uniqueAssociated.size()) {
    ...
    for (const auto& assoc : allAssociated) {
      std::cout << " pdgId " << assoc.pdgId() << " pt " << assoc.pt() << " eta " << assoc.eta() << std::endl;
    }
...
void DressedGenParticlesProducer::removeDoubleCounting(reco::CandidateCollection& dressedParticles) {
  for (size_t i = 0; i < dressedParticles.size(); i++) {
    const DressedGenParticle& dressed_part_i = dynamic_cast<const DressedGenParticle&>(dressedParticles[i]);
    reco::GenParticleCollection associates = dressed_part_i.getAssociated();
    for (size_t j = i + 1; j < dressedParticles.size(); j++) {
      const DressedGenParticle& dressed_part_j = dynamic_cast<const DressedGenParticle&>(dressedParticles[j]);
      ...
      std::cout << std::endl
                << " pdgId " << dressed_part_j.pdgId() << " pt " << dressed_part_j.pt() << " eta "
                << dressed_part_j.eta() << std::endl;
      for (const auto& assoc : associates) {
        std::cout << "Associated = " << dressed_part_j.isAssociated(assoc);
        std::cout << std::endl
                  << " pdgId " << assoc.pdgId() << " pt " << assoc.pt() << " eta " << assoc.eta() << std::endl;
      }
