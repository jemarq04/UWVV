...
PATJetIDEmbedder::PATJetIDEmbedder(const edm::ParameterSet& iConfig)
  ...
  if (!checkfile.good())
    idFileName_ = idFileName_.substr(idFileName_.find("/UWVV/") + 6);
  else
    checkfile.close();
...
void PATJetIDEmbedder::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (size_t i = 0; i < in->size(); ++i) {
    ...
    float passTightLepVeto =
        idFile_->at(lepvetoConfig_)->evaluate({eta, chHF, neHF, chEmEF, neEmEF, muEF, chMult, neMult, mult});
