...
PATObjectCounter<T>::PATObjectCounter(const edm::ParameterSet& iConfig)
    : srcTag_(iConfig.getParameter<edm::InputTag>("src")),
      ...
      cut_strings_(iConfig.exists("cuts") ? iConfig.getParameter<std::vector<std::string>>("cuts")
                                          : std::vector<std::string>()),
      labels_(iConfig.exists("labels") ? iConfig.getParameter<std::vector<std::string>>("labels")
                                       : std::vector<std::string>()),
      verbose_(iConfig.exists("verbose") ? iConfig.getParameter<bool>("verbose") : false),
      doPrintInfo_(iConfig.exists("printInfo") ? iConfig.getParameter<bool>("printInfo") : false) {
  if (cut_strings_.size() != labels_.size())
    throw cms::Exception("InvalidParams") << "You must supply an equal number of labels and cuts" << std::endl
                                          << "Given: labels_->size() == " << labels_.size()
                                          << "; cut_strings_->size() == " << cut_strings_.size() << std::endl;
...
void PATObjectCounter<T>::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (size_t i = 0; i < cut_strings_.size(); i++) {
    ...
    if (cut_strings_[i] != "") {
      ...
      *num = in->size();
    if (verbose_)
      std::cout << srcTag_.label() << " Count: " << labels_[i] << " = " << *num << std::endl;
