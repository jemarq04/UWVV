...
PATMuonZZIDEmbedder::PATMuonZZIDEmbedder(const edm::ParameterSet& iConfig)
    : muonCollectionToken_(consumes<MuonView>(iConfig.exists("src") ? iConfig.getParameter<edm::InputTag>("src")
...
void PATMuonZZIDEmbedder::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (MuonView::const_iterator mi = muonsIn->begin(); mi != muonsIn->end(); mi++)  // loop over muons
  {
    ...
    mu.addUserFloat(idLabel_ + "Tight",
                    float(idResult && (mi->isPFMuon() || trackerHighPtID)));  //PAS2019 version of TightMuonID
    mu.addUserFloat(idLabel_ + "TightNoVtx",
                    float(idResultNoVtx && (mi->isPFMuon() || trackerHighPtID)));  //PAS2019 version of TightMuonID
...
bool PATMuonZZIDEmbedder::passKinematics(const Muon& mu) const { return mu.pt() > ptCut && fabs(mu.eta()) < etaCut; }

bool PATMuonZZIDEmbedder::passVertex(const Muon& mu) const {
  return fabs(mu.dB(Muon::PV3D) / mu.edB(Muon::PV3D)) < sipCut && fabs(mu.dB(Muon::PV2D)) < pvDXYCut &&
         fabs(mu.dB(Muon::PVDZ)) < pvDZCut;
}
