...
PATObjectValueMapProducer<T>::PATObjectValueMapProducer(const edm::ParameterSet& iConfig)
    : srcToken(consumes<edm::View<T>>(iConfig.getParameter<edm::InputTag>("src"))),
      intVals_(iConfig.exists("intVals") ? iConfig.getParameter<std::vector<int>>("intVals") : std::vector<int>()),
      boolVals_(iConfig.exists("boolVals") ? iConfig.getParameter<std::vector<int>>("boolVals") : std::vector<int>()),
      doubleVals_(iConfig.exists("doubleVals") ? iConfig.getParameter<std::vector<double>>("doubleVals")
                                               : std::vector<double>()),
      floatVals_(iConfig.exists("floatVals") ? iConfig.getParameter<std::vector<double>>("floatVals")
                                             : std::vector<double>()),
  ...
  if (intVals_.size() != intLabels_.size())
    throw cms::Exception("InvalidParams") << "You must supply exactly one label for each int you want to embed"
                                          ...
                                          << "; intVals_.size() == " << intVals_.size() << std::endl;
  ...
  if (boolVals_.size() != boolLabels_.size())
    throw cms::Exception("InvalidParams") << "You must supply exactly one label for each bool you want to embed"
                                          ...
                                          << "; boolVals_.size() == " << boolVals_.size() << std::endl;
  ...
  if (doubleVals_.size() != doubleLabels_.size())
    throw cms::Exception("InvalidParams") << "You must supply exactly one label for each double you want to embed"
                                          ...
                                          << "; doubleVals_.size() == " << doubleVals_.size() << std::endl;
  ...
  if (floatVals_.size() != floatLabels_.size())
    throw cms::Exception("InvalidParams") << "You must supply exactly one label for each float you want to embed"
                                          ...
                                          << "; floatVals_.size() == " << floatVals_.size() << std::endl;
}
