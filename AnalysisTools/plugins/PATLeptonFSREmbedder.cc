...
PATLeptonFSREmbedder::PATLeptonFSREmbedder(const edm::ParameterSet& iConfig)
    : candToken_(consumes<PackedCandidateView>(iConfig.getParameter<edm::InputTag>("candidates"))),
      ...
      ptCut_(iConfig.exists("ptCut") ? iConfig.getParameter<double>("ptCut") : 2.),
      etaCut_(iConfig.exists("etaCut") ? iConfig.getParameter<double>("etaCut") : 2.5),
      drEtCut_(iConfig.exists("drEtCut") ? iConfig.getParameter<double>("drEtCut") : 0.012),
      deltaRMax_(iConfig.exists("deltaRMax") ? iConfig.getParameter<double>("deltaRMax") : 0.5),
      isoCut_(iConfig.exists("isoCut") ? iConfig.getParameter<double>("isoCut") : 1.8),
      chIsoConeMin_(iConfig.exists("chIsoConeMin") ? iConfig.getParameter<double>("chIsoConeMin") : 0.0001),
      chIsoPtCut_(iConfig.exists("chIsoPtCut") ? iConfig.getParameter<double>("chIsoPtCut") : 0.2),
      nIsoConeMin_(iConfig.exists("nIsoConeMin") ? iConfig.getParameter<double>("nIsoConeMin") : 0.01),
      nIsoPtCut_(iConfig.exists("nIsoPtCut") ? iConfig.getParameter<double>("nIsoPtCut") : 0.5),
      isoConeMax_(iConfig.exists("isoConeMax") ? iConfig.getParameter<double>("isoConeMax") : 0.3),
      drSafe_(0.0001),
      eCut_(iConfig.exists("eCut") ? iConfig.getParameter<std::string>("eCut") : ""),
      muCut_(iConfig.exists("muCut") ? iConfig.getParameter<std::string>("muCut") : "") {
...
double PATLeptonFSREmbedder::getIso(const PackedCandidate& photon, const PackedCandidateView& cands) {
  ...
  for (const auto& cand : cands) {
    ...
    //Charged hadrons
    if (cand.charge() != 0 && std::abs(cand.pdgId()) == 211 && cand.pt() > chIsoPtCut_ &&
        deltaR2 > chIsoConeMin_ * chIsoConeMin_)
      ptsum += cand.pt();
    //Neutral hadrons and photons
    else if (cand.charge() == 0 && (std::abs(cand.pdgId()) == 22 || std::abs(cand.pdgId()) == 130) &&
             cand.pt() > nIsoPtCut_ && deltaR2 > nIsoConeMin_ * nIsoConeMin_)
      ptsum += cand.pt();
  }
...
bool PATLeptonFSREmbedder::electronFootprintVeto(const PackedCandidateRef& phoRef,
                                                 const edm::Handle<ElectronCollection>& electronsForVeto) {
