...
class AnalysisStep(object):
                ...
                try:
                    ...
                    if module._seq is None or module._seq._collection is None:
                        raise AttributeError(
                            ("I don't know how to extract an input tag from the sequence in ") + name
                        ) from err
                    raise
            else:
    ...
    def addBasicSelector(self, obj, selection, name="", objectType="", newCollection=""):
        """
        ...
        self.addModule(
            "".join([obj, newCollection, name if name else "cleaning", self.name]).replace("_", ""), mod, collection
        )
    ...
    def addCrossSelector(self, obj, selection, name="", **otherObjects):
        ...
        self.addModule("".join([obj, name if name else "crossCleaning", self.name]).replace("_", ""), mod, obj)
