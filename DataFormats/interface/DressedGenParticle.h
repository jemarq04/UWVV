...
public:
  ...
  DressedGenParticle(const LeafCandidate& c) : reco::GenParticle(c) {}
  DressedGenParticle(const reco::GenParticle& c) : reco::GenParticle(c) {}
  DressedGenParticle(const reco::GenParticle& cand,
                     //const reco::GenParticleRefVector associates, float dRmax);
                     const reco::GenParticleCollection associates,
                     float dRmax);
  DressedGenParticle(Charge q, const LorentzVector& p4, const Point& vtx, int pdgId, int status, bool integerCharge);
  DressedGenParticle(
