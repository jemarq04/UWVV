...
DressedGenParticle::DressedGenParticle(
    Charge q, const LorentzVector& p4, const Point& vtx, int pdgId, int status, bool integerCharge)
    : reco::GenParticle(q, p4, vtx, pdgId, status, integerCharge), p4_undressed(p4) {}
DressedGenParticle::DressedGenParticle(
...
DressedGenParticle* DressedGenParticle::clone() const { return new DressedGenParticle(*this); }
const reco::Candidate::LorentzVector DressedGenParticle::undressedP4() const { return p4_undressed; }
float DressedGenParticle::undressedPt() const { return p4_undressed.pt(); }
float DressedGenParticle::undressedEta() const { return p4_undressed.eta(); }
float DressedGenParticle::undressedPhi() const { return p4_undressed.phi(); }
float DressedGenParticle::numAssociated() const { return associates.size(); }
reco::GenParticleCollection DressedGenParticle::getAssociated() const { return associates; }
...
bool DressedGenParticle::isAssociated(const reco::GenParticle& associated) const {
  ...
  std::vector<reco::GenParticle>::const_iterator entry =
      std::find_if(associates.begin(), associates.end(), [&associated](const reco::GenParticle& part) {
        return part.pt() == associated.pt();
      });
  return (entry != end);
}
